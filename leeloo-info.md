...
│   ├── doc.html/js         — вбудована документація (статичний контент)
...
5. По завершенню (`finalize()`) — фінальний `memory.txt`, побудова системного промпту (`buildSystemPrompt`), інʼєкція в активну вкладку моделі через `chrome.scripting.executeScript`. **Важливо:** `buildSystemPrompt()` вставляє лише шлях + опис структури папок + інструкцію "де що читати" — сам вміст `memory.txt`/`short_*`/`full_*` у промпт не підмішується (перевірено по коду, рядок ~759). Тобто це "мапа архіву", а не памʼять по факту: модель бачить, де лежать файли, але не має інструменту їх прочитати в звичайній chat-сесії (claude.ai/gemini.google.com/chatgpt.com не дають file-read tools). Реальний контент-injection наразі відсутній — є лише ручний обхід через фічу `createBook()` (розділ 3.3), яку користувач експортує й вставляє сам.
...
- **System prompt = мапа архіву, не памʼять.** `buildSystemPrompt()` (SW, ~рядок 759) вставляє шлях + структуру папок + стратегію читання, але не сам вміст `memory.txt`. Модель у звичайній chat-сесії (claude.ai/gemini.google.com/chatgpt.com) не має інструменту прочитати файл за цим шляхом — тож автоматичного "спрацював чи ні" memory injection зараз немає, є лише мапа-підказка. Реальний retrieval (вставка вмісту memory.txt/релевантних summaries в контекст) — заплановано, не реалізовано. Ручний обхід є через `createBook()` (розділ 3.3). Виявлено 08.07 через зовнішню рецензію Leeloo.
